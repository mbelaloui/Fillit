...
/*   Updated: 2017/12/15 19:37:03 by mbelalou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_rem_tetri(char ***mat, t_tetri *t, int x, int y)
{
	int pt;
	...
	while (point->x < dim_mat && t)
	{
		...
		while (point->y < dim_mat && t)
		{
			if (ft_is_possible(*mat, t, dim_mat, point))
			{
				...
				if (ft_put_tetrim_mat(mat, t->next, dim_mat))
					return (1);
				else
					ft_rem_tetri(mat, t, point->x, point->y);
			}
			point->y++;
		}
		point->x++;
